...
const Doctors = () => {
  ...
  const applyFilter = () => {
    if (speciality) {
      // Convert both to lowercase and trim extra spaces for comparison
      setFilterDoc(doctors.filter((doc) => doc.speciality.toLowerCase().trim() === speciality.toLowerCase().trim()));
    } else {
  ...
  return (
    <div>
      <p className="text-gray-600">Browse the Doctors by Speciality</p>
        ...
        <div className="w-full grid grid-cols-auto gap-4 gap-y-6">
          {
            filterDoc.map((item, index) => (
              <div onClick={()=> navigate(`/appointment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index}>
                <img className='bg-blue-50 w-full h-48 object-cover mb-4' src={item.image} alt={item.name} />
                <div className='p-4'>
